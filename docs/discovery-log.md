...
- Third-party review: https://kiokutokiroku.com/starfilter/

#### Verified facts

- 楽天市場の「便利グッズ」ランキングで1位として掲載されていた。
- 公式楽天店の商品ページでは、不燃性ガラス繊維の三層構造と、対応型番・サイズ確認の必要性が案内されている。
- 楽天の店舗レビューは確認時点で4.77、6,126件だった。これは店舗全体の評価であり、対象商品単体の評価ではない。
- 第三者レビューでは、サイズ不一致が購入時の注意点として挙げられている。

#### LIFT interpretation

- 換気扇内部を汚れてから掃除するより、油煙が入る前に防ぐ「予防型」の商品で、LIFTの思想と合う。
- 排気口カバーと悩みの領域が近いため、連続投稿すると内容が重複して見えるリスクがある。
...
- 主訴求は「掃除がラク」ではなく、「換気扇内部へ油を入れない」にする。
- サイズ確認は商品の欠点ではないため、「惜しい点」ではなく購入前チェックとして短く記載する。
- サイズを間違えた購入者への評価は推測になるため、公開文には含めない。
- 実体験にない欠点を、レビューから無理に補って掲載しない。

#### Publication blocker

- ユーザーの提示URLと楽天購入履歴から、実際の商品が `sfset2` の枠2枚用と一致した。
- 商品画像の利用条件と、対応する楽天アフィリエイトリンクを確認してから公開へ進む。
## 2026-08-23 delayed daily run
...
- Review source:
  https://review.rakuten.co.jp/item/1/227333_10017398/1.1/

#### Verified facts

- The signed-in Rakuten purchase history identifies the purchased variation as the white two-piece set. Private order identifiers are not stored.
- The product page lists model numbers 1308 / 1309, compatibility with approximately 6-8 cm diameter bottles, and use on magnetic surfaces.
- The item review page showed 4.68 from 234 reviews at the time checked.
...
### Candidate: 無印良品 やわらかポリエチレンケース

- Status: adopted and published as `lift-051`.
- Product: https://item.rakuten.co.jp/mujirushi-ryohin/4549738749479/
- Experience: 使用済み。洗面所では入浴後の着替え入れ、収納庫では洗剤・シャンプー・ボディソープなどのストック整理に使用。高さの違うサイズを用途に合わせて使い分けている。今のところ、特に惜しい点はない。
- Duplicate check: 商品台帳、公開履歴、楽天ROOM内検索で未公開を確認。
