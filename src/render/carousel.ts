...
export function renderCarousel(product: ProductRecord): RenderedPage[] {
  ...
  const pages = [
    ...
    `<main class="page">${brandMark}<div class="eyebrow">困りごと</div><section class="text-block"><h2>掃除しにくい場所ほど、\n汚れる前に守る。</h2><p class="body">${problem}</p></section></main>`,
    `<main class="page">${brandMark}<div class="eyebrow">変わったこと</div><section class="text-block"><h2>掃除を減らす仕組みに\n変える。</h2><p class="body">${change}</p></section></main>`,
