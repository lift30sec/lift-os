...
export function renderCarousel(product: ProductRecord): RenderedPage[] {
  ...
  const pages = [
    ...
    `<main class="page">${brandMark}<div class="eyebrow">困りごと</div><h2>掃除しにくい場所ほど、汚れる前に守る。</h2><p class="body">${problem}</p><div class="footer">毎日を30秒ラクにする。</div></main>`,
    `<main class="page">${brandMark}<div class="eyebrow">変わったこと</div><h2>掃除を減らす仕組みに変える。</h2><p class="body">${change}</p><div class="footer">${label}</div></main>`,
